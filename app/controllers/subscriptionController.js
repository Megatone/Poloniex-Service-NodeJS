...
function getActiveSubscriptions(nowTimestamp) {
    ...
    return Subscription.find({ status: true, lastNotification: { $lt: filterLastNotification } }).populate({ path: 'device' }, {}, {}, { 'device.notificationStatus': true }).populate({ path: 'pair' }).exec();
}
