...
function getActiveSubscriptions(nowTimestamp) {
    ...
    return Subscription.find({ status: true, lastNotification: { $lt: filterLastNotification } }).populate({ path: 'device', match: { 'device.notificationStatus': true } }).populate({ path: 'pair' }).exec();
}
