...
function getActiveSubscriptions(nowTimestamp) {
    ...
    return Subscription.find({ status: true, lastNotification: { $lt: filterLastNotification }, 'device.notificationStatus': true }).populate({ path: 'device' }).populate({ path: 'pair' }).exec();
}
